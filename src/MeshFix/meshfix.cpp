...
extern "C" int fixMesh(
    const char* infilename,
    const char* outfilename,
    bool join_multiple_components,
    bool stl_output,
    bool skip_if_fixed
) {
    TMesh::init();
    ...
    char extension[16];
    strcpy(extension, stl_output ? ".stl" : ".off");
    char outfilename_buf[2048];
    if (!outfilename || strlen(outfilename) == 0) {
        createFilename(infilename, "_fixed", outfilename_buf, extension);
        outfilename = outfilename_buf;
    }

    if (skip_if_fixed && fopen(outfilename, "r")) {
        TMesh::error("Output file already exists (-x option specified).");
        return 2;
    }

    Basic_TMesh tin;
    if (tin.load(infilename) != 0) {
        TMesh::error("Can't open file.\n");
        return 1;
    }
